...
function BoxShadowComponent(props) {
  ...
  //update 2017-03-04
  //format the color
  const rgb = React.useCallback(colorRgb(color));

  //the same parts for gradients
  const linear = function(key) {
    return [
      <Stop
        offset="0"
        stopColor={color}
...
export const BoxShadow = React.memo(BoxShadowComponent);
